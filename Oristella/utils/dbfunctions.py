...
async def is_using_Oristella(user_id):
...
async def add_user(user_id):
    is_using = await is_using_Oristella(user_id)
    if is_using:
        return
    return await usersdb.insert_one({"user_id": user_id}) 
...
async def is_Oristella_in_groups(group_id):
...
async def add_group(group_id):
    is_using = await is_Oristella_in_groups(group_id)
    if is_using:
        return
