...
from Oristella import app,date
from Oristella.plugins.wishper import lengths
...
from Oristella.mongo.feddb import (
    get_fed_from_ownerid,
    get_fed_name,
    fed_promote,
)
from Oristella.mongo.approvedb import Approve
from Oristella.mongo.disabledb import Disabling
from Oristella.mongo.filterdb import Filters
from Oristella.mongo.notesdb import Notes
from Oristella.mongo.blacklistdb import Blacklist
from Oristella.mongo.warnsdb import WarnSettings
from Oristella.mongo.rulesdb import Rules
from Oristella.mongo.warnsdb import Warns
from Oristella.mongo.captcha import captchas
from Oristella.mongo.welcomedb import Greetings
from Oristella.mongo.connectiondb import(
    all_connections,
    if_active,
    make_active,
    delete_connection,
...
from Oristella.utils.string import (
    build_keyboard,
    parse_button
)
from Oristella.core.decorators.permissions import member_permissions
...
async def verify_(bot,query):
        ...
        if chat:
            ...
            msg = await bot.send_photo(
                    chat_id=chat_id,
                    photo=data_["captcha"],
                    caption=f"{query.from_user.mention}[{query.from_user.id}] Please click on each {typ_} button that is shown in image",
...
async def off_(bot,query):
        ...
        if query.from_user.id not in localdatabase:
            return await query.answer("Start me in PM & Try Again After Re-Join!", show_alert=True)
