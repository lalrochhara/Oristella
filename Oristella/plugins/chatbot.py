...
from Oristella import app,BOT_ID
from Oristella.utils.filter_groups import cbot
from lang import get_command
from Oristella.utils.lang import language
from Oristella.mongo import chatb
from Oristella.plugins.antlangs import get_arg
from Oristella.utils.custom_filters import admin_filter
...
async def Thecbot(_, message: Message):
    ...
    if chat:
        ...
        try:
           ...
           affiliateplus = requests.get(f"https://api.affiliateplus.xyz/api/chatbot?message={text}&botname=Oristella&ownername=@NickyLrca&user={chat_id}")
           textmsg = (affiliateplus.json()["message"])
           if "Affiliate+" in textmsg:
               textmsg = textmsg.replace("Affiliate+", "Oristella")
           if "Lebyy_Dev" in textmsg:
               textmsg = textmsg.replace("Lebyy_Dev", "Nicky Lalrochhara")
           if "God Brando" in textmsg:
               textmsg = textmsg.replace("Famhawite Infosys", f"{message.from_user.first_name}")
           ...
           trtoen = (message.text if lang=="en" else tr.translate(message.text, dest="en").text).replace(" ", "%20")
           text = trtoen.replace(" ", "%20") if len(message.text) < 2 else trtoen
           safeone = requests.get(f"https://api.safone.tech/chatbot?message={text}&bot_name=Oristella&bot_master=NickyLrca&user_id={user_id}")
           textmsg = (safeone.json()["answer"])
           if "Affiliate+" in textmsg:
               textmsg = textmsg.replace("Affiliate+", "Oristella")
           if "[Safone]" in textmsg:
               textmsg = textmsg.replace("[Safone]", "Nicky Lalrochhara")
           msg = tr.translate(textmsg, src='en', dest=lang)
           await message.reply_text(msg.text)
...
AI based chatbot allows Oristella to talk and provides a more interactive group chat experience.
...
**Language Support**
Oristella AI chatbot support almost all languages in world .
